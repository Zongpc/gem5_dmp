    ...
    pf_helper(nullptr)
{
    ...
    */
    ...
    // DPRINTF(
    //         DMP, "the size of iddt_ent_t %d \n",
    //         sizeof(iddt_ent_t)
    // );
    // DPRINTF(
    //         DMP, "the size of tadt_ent_t %d \n",
    //         sizeof(tadt_ent_t)
    // );
    // DPRINTF(
    //         DMP, "the size of RangeTableEntry %d \n",
    //         sizeof(RangeTableEntry)
    // );
    // DPRINTF(
    //         DMP, "the size of IndexQueueEntry %d \n",
    //         sizeof(IndexQueueEntry)
    // );
    // DPRINTF(
    //         DMP, "the size of ICSEntry %d \n",
    //         sizeof(ICSEntry)
    // );
    // DPRINTF(
    //         DMP, "the size of RTEntry %d \n",
    //         sizeof(RTEntry)
    // );
...
DiffMatching::pickIndexPC()
{
    ...
    // schedule next try
    if (auto_detect) {
        schedule(checkNewIndexEvent, curTick() + clockPeriod() * detect_period);//xymc
    }
}
...
DiffMatching::notifyICSMiss(Addr miss_addr, Addr miss_pc_in, ContextID cID_in)
{
    for (auto& ics_ent : indirectCandidateScoreboard) {
        ...
        if((miss_pc_in & 0xffff800000000000) != 0)
        //||(miss_pc_in & 0x7fbf000000) != 0)
            continue;
        DPRINTF(DMP, "ICS updateMiss: indexPC %llx targetPC %llx Addr %llx cID %d\n",ics_ent.index_pc,miss_pc_in, miss_addr, cID_in);
...
DiffMatching::diffMatching(tadt_ent_t& tadt_ent)
{
    ...
    for (const auto& iddt_ent : indexDataDeltaTable) {
        ...
        for (int i_start = 0; i_start < iddt_diff_num-tadt_diff_num+1; i_start++) {
            ...
            //xymc
            // if(iddt_ent[i_start]%tadt_ent[0]!=0)
            //     continue;
            // unsigned int shift_try=iddt_ent[i_start]/tadt_ent[0];
            // int t_start=1;
            // while (t_start < tadt_diff_num) {
            //     if (iddt_ent[i_start+t_start]*shift_try != tadt_ent[t_start] ) {
            //         break;
            //     }
            //     t_start++;
            // }
...
bool
DiffMatching::findRTE(Addr index_pc, const tadt_ent_t& tadt_ent_match, ContextID cID)
{
    ...
    int iddt_match_point, unsigned int shift, ContextID cID)
{
    ...
    uint64_t base_addr_tmp = addr_match - (data_match << shift);

    DPRINTF(DMP, "Matched: LastData %llx Data %llx Addr %llx Shift %d\n",
               iddt_ent_match.getLast(), data_match, addr_match, shift);
    //xymc int64_t->uint64_t
    assert(base_addr_tmp <= static_cast<uint64_t>(std::numeric_limits<uint64_t>::max()));
...
DiffMatching::RangeTableEntry::updateSample(Addr addr_in)
{
    ...
    {
        return false;//xymc:Why do you need to judge two here
    }
...
DiffMatching::notifyL1Req(const PacketPtr &pkt)
{
    ...
    /*xymc:Check whether the difference between this address and
    the last data obtained from the same PC is less than 4K*/
    // for (auto& iddt_ent: indexDataDeltaTable) {
    //     Addr target_pc = iddt_ent.getPC();

    //     if (target_pc != pkt->req->getPC() || !iddt_ent.isValid()) continue;
    //     // DPRINTF(DMP,"The difference between the PC and the address PC %llx, Addr %llx, Value %llx\n",target_pc,
    //     // req_addr,iddt_ent.getLast());
    //     if(req_addr-iddt_ent.getLast()<4096)
    //     {
    //         DPRINTF(DMP,"It is judged to be a linked list pointer target PC %llx index PC %llx\n",target_pc,target_pc);
    //         insertTADT(target_pc, iddt_ent.getContextId());
    //     }
    // }
...
DiffMatching::notifyL1Resp(const PacketPtr &pkt)
{
    ...
    **/
    ...
    //update TADT to detect pointer chase
    for (auto& tadt_ent: targetAddrDeltaTable) {
        ...
        // tadt_ent validation check
        if (target_pc != pkt->req->getPC() || !tadt_ent.isValid()) continue;
        // if(target_pc!=0x40116c)
        //     continue;
        if(tadt_ent.isFinish())
            continue;
        IndexData new_data;
        std::memcpy(&new_data, &resp_data, sizeof(int64_t));
        DPRINTF(POINTER,"update TADT:PC %llx, last_addr %llx, Data %llx\n",target_pc,tadt_ent.getLast(),resp_data);
...
DiffMatching::notifyFill(const PacketPtr &pkt, const uint8_t* data_ptr)
{
    ...
    for (const auto& rt_ent: relationTable) {
        ...
        // for (unsigned i_of = rt_ent.range?std::min((data_offset + data_stride * (rt_ent.range_degree-1)),blkSize)
        // :data_offset; i_of < range_end; i_of += data_stride)
        for (unsigned i_of=data_offset;i_of < range_end;i_of += data_stride)
        {
            ...
            DPRINTF(HWPrefetch,
                    "notifyFill: PC %llx, pkt_addr %llx, pkt_offset %llx, pkt_data %llx, pf_addr %llx\n",
            ...
            // if (rt_ent.target_pc == 0x400ca0) {//xymc
...
DiffMatching::notify (const PacketPtr &pkt, const PrefetchInfo &pfi)
{
    if (pfi.isCacheMiss()) {
        ...
        // {
        //     DPRINTF(HWPrefetch, "notify::CacheMiss: prefetch PC %llx, Addr %llx, PAddr %llx, VAddr %llx\n",
        //                         pkt->req->hasPC() ? pkt->req->getPC() : 0x0,
        //                         pkt->getAddr(),
        //                         pkt->req->getPaddr(),
        //                         pkt->req->hasVaddr() ? pkt->req->getVaddr() : 0x0);
        ...
        // if(pkt->req->isPrefetch())
        // {
        //     DPRINTF(HWPrefetch, "notify::CacheHit: prefetch PC %llx, Addr %llx, PAddr %llx, VAddr %llx\n",
        //                         pkt->req->hasPC() ? pkt->req->getPC() : 0x0,
        //                         pkt->getAddr(),
        //                         pkt->req->getPaddr(),
        //                         pkt->req->hasVaddr() ? pkt->req->getVaddr() : 0x0);
...
//xymc:How this function is called
void
