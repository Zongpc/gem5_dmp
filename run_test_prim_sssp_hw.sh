...
# 定义 -r 参数的值
r_values=("graph_prim_ss" "graph_sssp_ss")
...
for r in "${r_values[@]}"; do
    ...
    if [ -d "m5out/${r}" ]; then
        # 定义第一个指令
        first_command="build/ARM/gem5.opt --debug-flags=HWPrefetch \
        configs/dmp_pf/fs_L2.py --num-cpus 1 --cpu-clock 2.5GHz --cpu-type O3_ARM_v7a_3 \
        --caches --l2cache --l1i_assoc 8 --l1d_assoc 8 --l2_assoc 4 --l2_mshr_num 32 \
        --l2_repl_policy LRURP ${dmp_flag} --dmp-notify l1 \
        --mem-type SimpleMemory --mem-size 8GB --kernel=/home/zongpc/work/qiang/os/binaries/vmlinux.arm64 \
    ...
    # 执行第一个指令
    echo "执行指令: $first_command"
    eval $first_command  > tmplog_${r}_HWPrefetch.txt
    ...
    # 移动并重命名 stats.txt 文件
    mv "m5out/stats.txt" "m5out/${r}/stats_${r}_HWPrefetch.txt"
    mv "m5out/system.terminal" "m5out/${r}/${r}_HWPrefetch.terminal"
done
